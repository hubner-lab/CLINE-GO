...
config_schema <- function() {
    ...
    list(
        ...
        s("Population.calc_stats",       "Calculate pop stats", "structure", "Pop Stats",
          "checkbox", TRUE,
          help = "Compute Tajima's D, Pi diversity and AMOVA per population"),
        ...
        s("haplotype.scan.metadata_type",     "Metadata type",     "haplotype", "Scan Parameters",
          "select",  FALSE,
          choices = c("site", "cluster"),
          help = "Metadata grouping for haplotype annotation: sampling site, or the sNMF ancestry cluster (argmax of the K-best Q matrix)"),
        ...
        s("Maladaptation.methods.gradient_forest.extrap",
          "Linear extrapolation", "maladaptation","Gradient Forest",
          "checkbox", FALSE,
          help = paste0("How the offset is projected onto raster cells whose climate lies OUTSIDE the ",
                        "training range. On (gradientForest default, what published GF-offset code runs): ",
                        "each turnover function continues as a straight line at its boundary slope. Off: ",
                        "the predictor is clamped to the range limit, so the function plateaus (conservative, ",
                        "underestimates offset in novel climate). The manual calls extrapolation experimental; ",
                        "the share of cells outside the training range is reported in the model badge either way.")),
