...
#'
#' TOTAL with respect to its input: one output row per input string, always.
#' An empty or NA attribute field yields an all-NA row rather than being
#' dropped. That is the contract load_gff_genes()'s positional cbind assumes;
#' before 2026-09-13 rbindlist silently discarded the empty lists, the result
#' was shorter than the input, and the attributes recycled onto the wrong genes
#' with no warning when the lengths happened to divide evenly.
#'
#' @param attr_strings character vector of raw GFF attribute strings
#' @return data.table with one column per attribute key and
#'   length(attr_strings) rows. Zero columns AND zero rows when no row carries
#'   any attribute at all — there is nothing to bind, and a caller indexing a
#'   column by name gets NULL rather than an error.
#' @noRd
parse_gff_attributes <- function(attr_strings) {
    ...
    # No keys anywhere: no columns to bind, so no rows either.
    if (!any(lengths(parsed) > 0L)) return(data.table::data.table())

    # Give every empty row a full set of NA keys, so rbindlist keeps it. A
    # sentinel column would change the output schema; this does not.
    keys  <- unique(unlist(lapply(parsed, names), use.names = FALSE))
    blank <- stats::setNames(rep(list(NA_character_), length(keys)), keys)
    parsed[lengths(parsed) == 0L] <- list(blank)
...
#' fields into columns. Cached per project AND per GFF mtime to avoid re-loading
#' the full GFF on every reactive trigger without going stale when the pipeline
#' regenerates it.
#'
#' @param project project name
...
load_gff_genes <- function(project, config) {
    # Resolve the path OUTSIDE load_cached() so the entry can be fingerprinted.
    # Without a fingerprint the key is sticky for the life of the session: a GFF
    # regenerated by a pipeline re-run was never picked up and the Region
    # Explorer served stale genes with no indication. Same pattern as
    # assign_region_ids() below.
    ...
    fp <- tryCatch(as.character(file.info(gff_abs)$mtime),
                   error = function(e) "unknown")

    load_cached(paste0("gff_genes_", project), function() {
        tryCatch({
            ...
            # Extract gene_id: prefer Parent=, fall back to ID=, strip isoform
            # suffixes. The PIPELINE's implementation, shared via
            # CLINEGO_SHARED_LIBS (zzz.R) — the app used to carry its own copy
            # built on regmatches(regexpr(...)), which returns only the elements
            # that MATCHED and so recycled a short vector across the rows.
            dt[, gene_id := extract_gene_id(attributes)]
            ...
            # The bind below is POSITIONAL. Guard it with its own message rather
            # than stopifnot(), so the handler's log line distinguishes a lost
            # row from a genuine read failure. ncol == 0 is the legitimate
            # "no attributes anywhere" case and binds to nothing.
            if (ncol(attr_dt) > 0L && nrow(attr_dt) != nrow(dt))
                stop("attribute parse lost rows: ", nrow(attr_dt),
                     " parsed vs ", nrow(dt), " features")
            cbind(dt[, .(gene_id, chr, start, end)], attr_dt)
        }, error = function(e) {
            # Do NOT return silently: an empty table here is indistinguishable
            # from "this region genuinely has no genes", which is how a parse
            # failure used to read as a biological negative.
            message("load_gff_genes: GFF parse failed — ", conditionMessage(e))
