...
**Extrapolation to novel climates.** Configurable (`Maladaptation.methods.gradient_forest.extrap`, 2026-09-13). `predict.gradientForest(extrap = TRUE)` — the package default and the pipeline default — continues each turnover function as a straight line at its boundary slope beyond the training range; `extrap = FALSE` clamps the predictor to the range limit ("flat" turnover, conservative). The manual calls extrapolation "an experimental feature" with no study of which kind is meaningful; every public GF-offset code base reached (Fitzpatrick's geneticOffsetR, the Gugger tutorial) runs the default by omission, and Rellstab et al. 2021 ask only that novel-climate predictions be flagged. `gradient_forest_diagnostics.tsv` records the setting and the share of raster cells outside the training envelope per predictor and scenario. *(Until the audit of 2026-09-13, SC1, this paragraph claimed the pipeline plateaus; it never did.)* **[verified]** (gradientForest 0.1-37 manual; Rellstab et al. 2021)
